    ...
    return (
        ...
        >
            <h3 className="mc__title text-base md:text-xl text-center">
                {status === "success" ? "Registration received" :
                    "Join our email list for project updates."}
            </h3>

            {status === "sending" && (
                <div
                    className="mc__alert mc__alert--sending text-center text-gvhGold-100 "
                >sending...</div>
            )}
            {status === "error" && (
                <div
...
{/* <InputField
...

            {
                status === 'success' ? '' : <InputField
                    label="subscribe"
                    type="submit"
                    formValues={[email, firstName, lastName, phone, postal, howdidu, pref, are_you_realtor, working_with, opt_in]}
                />
...
// https://nottinghilltowns.us18.list-manage.com/subscribe/post?u=839fb24cef0413753d27dbc3a&amp;id=3c128001c7
const MailchimpForm = props => {
    const url = `https://nottinghilltowns.us18.list-manage.com/subscribe/post-json?u=839fb24cef0413753d27dbc3a&amp;id=3c128001c7`;
